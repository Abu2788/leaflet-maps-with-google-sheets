$(window).on('load', function() {
  ...
  function mapPoints(points, layers) {
    ...
    if (displayTable && columns.length > 1) {
      ...
      $('#map').css('height', (100 - tableHeight) + 'vh'); // -------------------------------change to 100 to have the map appear, 0 - just the table, no map, or not?
      map.invalidateSize();
      ...
      var table = $('#maptable_dashboard').DataTable({
        paging: false,
        scrollCollapse: true,
          ...
          $.getJSON(
            apiUrl + spreadsheetId + '?key=' + googleApiKey
              ...
              $.when(
                ...
                $.getJSON(apiUrl + spreadsheetId + '/values/Dashboard?key=' + googleApiKey),
