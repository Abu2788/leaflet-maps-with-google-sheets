$(window).on('load', function() {
   ...
   * to specified (lat, lon) and zoom if all three are specified------------------------changed false to true for all, not to focus on points
   */
  function centerAndZoomMap(points) {
    var lat = map.getCenter().lat, latSet = true;
    var lon = map.getCenter().lng, lonSet = true;
  ...
  function mapPoints(points, layers) {
    ...
    if (displayTable && columns.length > 1) {
      ...
      $('#map').css('height', (100 - tableHeight) + 'vh'); // -------------------------------change to 100 to have the map appear, 0 - just the table, no map
      map.invalidateSize();
      ...
      // Update table every time the map is moved/zoomed or point layers are toggled--------------------to check here for column unalignment
      map.on('moveend', updateTable);
      map.on('layeradd', updateTable);
      map.on('layerremove', updateTable);
	 

      // Clear table data and add only visible markers to it-------------------changed----data is shown even if map is moved
      ...
      var table = $('#maptable_space').DataTable({
        paging: false,
        scrollCollapse: false,
		
        scrollY: 'calc(' + tableHeight + 'vh - 90px)',
  ...
  function addBaseMap() {
    ...
    L.control.attribution({prefix: 'some text'}).addTo(map);
  }
         ...
         if (typeof googleApiKey !== 'undefined' && googleApiKey) {
          ...
          $.getJSON(
            apiUrl + spreadsheetId + '?key=' + googleApiKey
              ...
              // First, read 3 sheets: options, Points, and Polylines--------------------------------------------------------------------changed points sheet into dashboard, removed polylines
              $.when(
                $.getJSON(apiUrl + spreadsheetId + '/values/options?key=' + googleApiKey),
                $.getJSON(apiUrl + spreadsheetId + '/values/Space?key=' + googleApiKey),
